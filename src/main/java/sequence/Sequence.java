 ...
 * <br>
 * 41位时间截(毫秒级)，注意，41位时间截不是存储当前时间的时间截，而是存储时间截的差值（当前时间截 - 开始时间截) 得到的值），这里的的开始时间截，一般是我们的id生成器开始使用的时间，由我们程序来指定的（如下START_TIME属性）。41位的时间截，可以使用69年，年T
 ...
 * <p>
 * <p>
 * 特性： 1.支持自定义允许时间回拨的范围<p> 2.解决跨毫秒起始值每次为0开始的情况（避免末尾必定为偶数，而不便于取余使用问题）<p> 3.解决高并发场景中获取时间戳性能问题<p> 4.支撑根据IP末尾数据作为workerId
...
public final class Sequence {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 10; i++) {
            System.err.println(sequence.nextId());
        }
    }
